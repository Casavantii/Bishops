...
void search(){
  ...
  while( (SensorValueRight/SensorValueLeft < 0.8 || SensorValueRight/SensorValueLeft > 1.2) ){
  read();
...
if (CenterButton == 0) { // motors deactivated if sleep button, or read button are pressed
  ...
  while  (SensorValueRight < 60 && SensorValueLeft < 60) {
    search();
  }
  
  
  while ( (SensorValueRight < 550 && SensorValueLeft < 550) && reachedGoal ==0){ // advance forward until goal voltage and while search is activated
    Robot.motorsWrite(255, 255); 
    delay(500);
    Robot.motorsWrite(0,0);
    read();
    if ( SensorValueRight/SensorValueLeft < 0.8 || SensorValueRight/SensorValueLeft > 1.2 ){ // verifying if still in good direction
    search();  
    delay(10); 
    }
  }  
  ...
  if ( (SensorValueRight > 550 && SensorValueLeft > 550) ){//&& proximGoal == 1) // switches to the proximity central eye and advances forward till goal voltage and while search is activated
